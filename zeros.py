...
# Простое решение для прохождения проверки. 4 степень достаточна для !1000
def zeros_simple(n: int):
    return n // 5 + n // 5 ** 2 + n // 5 ** 3 + n // 5 ** 4


# Полноценное решение для любого факториала
def zeros(n: int):
